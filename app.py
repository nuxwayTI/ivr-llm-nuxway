...
# CONFIG (ENV)
# =========================
# Base URL público de Render, ej:
# https://tu-servicio.onrender.com
BASE_URL = os.getenv("BASE_URL", "").strip().rstrip("/")

# Voz / idioma TTS
TTS_LANGUAGE = os.getenv("TTS_LANGUAGE", "es-MX").strip()
TTS_VOICE = os.getenv("TTS_VOICE", "Polly.Andres-Neural").strip()  # mejor calidad
TTS_USE_SSML = os.getenv("TTS_USE_SSML", "0").strip() == "1"

# Memoria TTL (segundos). 20 min por defecto.
...
last_seen = defaultdict(lambda: 0.0)  # TTL
...
def _maybe_ssml(text: str) -> str:
    if not TTS_USE_SSML:
        return text
    # SSML suave para teléfono (más claro)
    # Nota: Twilio suele aceptar SSML en <Say> con voces compatibles.
    safe = (text or "").replace("&", "y")
    return (
        f'<speak>'
        f'<prosody rate="92%" volume="+2dB">{safe}</prosody>'
        f'</speak>'
    )
...
def _abs_url(path: str) -> str:
    # Si BASE_URL existe, forma URL absoluta para Twilio (más confiable).
    if BASE_URL:
